...
def master(mode, pyomo, gekko):
    ...
    lowS = SizeSortedRates[(SizeSortedRates["doubleDAQlpGBT"]
                            == False)]["EvSize"][:int(nGBTSingles/3)].to_dict()
    medS = SizeSortedRates[(SizeSortedRates["doubleDAQlpGBT"] == False)]["EvSize"][int(len(SizeSortedRates[(SizeSortedRates["doubleDAQlpGBT"] == False)])/2) -
                                                                                   int(nGBTSingles/6):int(len(SizeSortedRates[(SizeSortedRates["doubleDAQlpGBT"] == False)])/2)+int(nGBTSingles/6)].to_dict()
    highS = SizeSortedRates[(SizeSortedRates["doubleDAQlpGBT"]
                             == False)]["EvSize"][-1*int(nGBTSingles/3):].to_dict()
    lowD = SizeSortedRates[(SizeSortedRates["doubleDAQlpGBT"] == True)
                           ]["EvSize"][:int(nGBTDoubles/3)].to_dict()
    medD = SizeSortedRates[(SizeSortedRates["doubleDAQlpGBT"] == True)]["EvSize"][int(len(SizeSortedRates[(SizeSortedRates["doubleDAQlpGBT"] == True)])/2) -
                                                                                  int(nGBTDoubles/6):int(len(SizeSortedRates[(SizeSortedRates["doubleDAQlpGBT"] == True)])/2)+int(nGBTDoubles/6)].to_dict()
    highD = SizeSortedRates[(SizeSortedRates["doubleDAQlpGBT"] == True)
                            ]["EvSize"][-1*int(nGBTDoubles/3):].to_dict()
    ...
    if mode == 'Lt':
        singleGBTRates = SizeSortedRates[(
            SizeSortedRates["doubleDAQlpGBT"] == False)]["EvSize"][:nGBTSingles].to_dict()
        doubleGBTRates = SizeSortedRates[(
            SizeSortedRates["doubleDAQlpGBT"] == True)]["EvSize"][:nGBTDoubles].to_dict()
    if mode == 'Ht':
        singleGBTRates = SizeSortedRates[(
            SizeSortedRates["doubleDAQlpGBT"] == False)]["EvSize"][-1*nGBTSingles:].to_dict()
        doubleGBTRates = SizeSortedRates[(
            SizeSortedRates["doubleDAQlpGBT"] == True)]["EvSize"][-1*nGBTDoubles:].to_dict()
...
def Pyomo(multiplier, singleGBTRates, doubleGBTRates):
    ...
    GKavSlinkRate = ((sum([i for i in singleGBTRates.values()]) +
                      sum([i for i in doubleGBTRates.values()]))/nSlinks)
    #maxSlinkRatePyomo = GKavSlinkRate*(multiplier)
    maxSlinkRatePyomo = max([i for i in singleGBTRates.values(
    )] + [i for i in doubleGBTRates.values()])*(multiplier)
    ...
    def OBJrule(model):
        ...
        for j in Slink:
            GKslinkRates = sum([singleGBTRates[int(i)]*model.GKSingles[i, j] for i in Single]) + \
                sum([doubleGBTRates[int(i)]*model.GKDoubles[i, j]
                     for i in Double])
    ...
    pyo.SolverFactory(
        '../PyomoSolvers/couenne-osx/couenne').solve(model, tee=False)
...
def Gekko(multiplier, singleGBTRates, doubleGBTRates, GekkoIter):
    ...
    GKslinkRates = [
        m.Intermediate(
            sum([singleGBTRates[i]*GKSingles[i, j]
                 for i in range(nGBTSingles)])
            + sum([doubleGBTRates[i]*GKDoubles[i, j]
                   for i in range(nGBTDoubles)]),
            name=f'outRate_{j}')
        for j in range(nSlinks)]
